...
async function moveFile(source, destination) {
  try {
    ...
    if (error && typeof error === "object" && "code" in error) {
      ...
      if (errorCode === "EXDEV") {
        await import_promises.default.copyFile(source, destination);
        await import_promises.default.unlink(source);
        return;
      }
    }
    throw error;
...
async function compressImagesToWebp({
  ...
  for (const filename of files) {
    ...
    const inputPath = import_path2.default.join(inputDir, filename);
    const outputName = `${import_path2.default.parse(filename).name}.webp`;
    const outputPath = import_path2.default.join(outputDir, outputName);
    const { size: sizeBefore } = await import_promises2.default.stat(inputPath);
    totalSizeBefore += sizeBefore;
    await sharp(inputPath).webp({ quality }).toFile(outputPath);
    const { size: sizeAfter } = await import_promises2.default.stat(outputPath);
...
var require_queue = __commonJS({
  "node_modules/.pnpm/fastq@1.20.1/node_modules/fastq/queue.js"(exports2, module2) {
    ...
    function fastqueue(context, worker, _concurrency) {
      ...
      function error(handler5) {
        errorHandler = handler5;
      }
    }
    function noop() {
    }
...
async function ensureDirectoryExists(dirPath, errorMessage) {
  const stats = await import_promises3.default.stat(dirPath).catch(() => null);
...
async function renameFiles({
  ...
  const targetDirectory = import_path3.default.dirname(targetPatternWithPath);
  await ensureDirectoryExists(
    targetDirectory,
    `Target directory does not exist: ${targetDirectory}`
  );
  const targetPattern = import_path3.default.basename(targetPatternWithPath);
  const files = await listFiles(srcDir);
  if (files.length === 0) {
    return { processed: 0, empty: true };
  }
  const tmpDir = await import_promises3.default.mkdtemp(import_path3.default.join(import_os.default.tmpdir(), "joyco-rename-"));
  const operations = [];
  try {
    let counter = 0;
    for (const filename of files) {
      const filePath = import_path3.default.join(srcDir, filename);
      ...
      const tempPath = import_path3.default.join(tmpDir, newFile);
      await import_promises3.default.copyFile(filePath, tempPath);
      if (onPrepare) {
      ...
      operations.push({
        sourcePath: filePath,
        tempPath,
        finalPath: import_path3.default.join(targetDirectory, newFile)
      });
      counter += 1;
    }
...
var pkgRoot = import_path5.default.resolve(__dirname, "..", "..");
var scriptsDir = import_path5.default.join(pkgRoot, "tools");
var definition4 = {
  name: "fix-svg",
...
function runCommand(bin, args, options = {}) {
  const result = (0, import_child_process.spawnSync)(bin, args, { stdio: "inherit", ...options });
  if (result.error) {
    throw result.error;
  }
  if (result.status !== 0) {
...
async function handler4(paths, options) {
  ...
  const files = await (0, import_fast_glob.default)(targetPaths, {
    onlyFiles: true,
    unique: true,
    ignore: ["**/node_modules/**"],
    extglob: true,
